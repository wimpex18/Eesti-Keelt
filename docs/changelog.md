...
Split out of `status.md` on 2026-09-12, where it was nine tenths of the file.
Everything here is a record of a past state and is true by construction: a
retrospective does not go stale, so nothing in this file needs re-checking
against the code. What *is* true now lives in `status.md`, which is the file to
re-check and the file to trust.

Kept whole rather than pruned. The entries are the evidence behind
`lessons.md` — a habit with no incident behind it is advice, and this project
has found that advice does not stick.
