...
| **Vasta küsimusele** (open answer) | free speech | transcript through the grammar chain and word count (pace is computed only when the client sends the duration, which it does not yet) | ASR + LLM, advisory |
