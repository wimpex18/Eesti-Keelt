...
| EKI A1/A2/B1 level word lists | **owner-fetchable, not code-fetchable** — the gate is a form asking who you are and what for, so the honest route is the learner downloads it and `cli ingest`s it. No importer is written for a file nobody has yet. See `content-sources.md`. |
