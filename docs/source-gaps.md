...
| EKI A1/A2/B1 level vocabulary | **wired 2026-09-11, absent in production** — `cli import-levels`, EKI's levels win in `words.proficiency` with `level_source` saying so. The merge-triggered build never has the file: `eki_levels` 0 in smoke run 34765657703 (2026-09-13) |
...
| EKI *põhisõnavara sõnastik* (`psv`) | **wired, never met its input** — `cli import-psv` and `psv.py` exist, and the Dockerfile runs the import. But the parser is tested only against a fixture built from the schema: the real XML is not on the operator machine (checked 2026-09-13) and cannot reach the production build (git-ignored; `eki_definitions` 0 in smoke run 34765657703). A parser written blind is still a code path that has never met its input; the first person to hold the file runs it before anything else. The ~6 000 pronunciation WAVs are not wired at all. |
