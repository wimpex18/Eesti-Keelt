...
| **Official tasks** | HARNO's own past tasks and listening recordings, downloaded with `cli harvest-exam --download` and opened inside `Eksam`: the PDF's text is extracted and read on the page, the recording plays there, and the file itself is one click away (`/api/exam/file`, `/api/exam/text`). A task that was not downloaded still links out to harno.ee. Private study only; every task carries © Haridus- ja Noorteamet. |
