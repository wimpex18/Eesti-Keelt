...
def reading_next(limit: int = 6, section: str = "lugemine") -> dict:
    ...
    note = (
        f"Сначала тексты, где больше всего посильных слов — знакомых тебе или "
    ...
    if picked["fallback"]:
        note = (
            ...
            f"Читай их со словарём: нажми на слово."
        )
    if not scored and unmeasurable:
        note = (
            "Словарный список не собран, поэтому долю посильных слов посчитать "
            "нельзя — это не значит, что тексты трудные. Собери его командами "
